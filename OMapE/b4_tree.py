...
class OMapTree(object):
    ...
    @staticmethod
    def create_tree(ident, branching_factor, elements):
        vector_length = len(elements)
    ...
    def add_child(self, parent_node, child):
        if parent_node is None or self.tree.get_node(parent_node) is None:
            return
    ...
    def new_node(self, current_node, parent_node, elements=None, leaf=False):
        ...
        if current_node == "root":
            ...
            if elements is not None and len(elements) > 0:
                ...
                self.add_child(parent_node, None)
    ...
    def build_tree(self, og_elements):
        ...
        elements = []
        for k in og_elements.keys():
           h = SHA256.new()
           h.update(str(k).encode('utf-8'))
           elements.append((h.digest(),og_elements[k]))
        ...
        while parent_node > 0:
            ...
            if parent_node_data is not None:
                ...
                lnode_data = None
                try:
                    lnode_data = self.get_node_data(child_1)
                except (TypeError, AttributeError):
                    pass
                rnode_data = None
                try:
                    rnode_data = self.get_node_data(child_2)
                except (TypeError, AttributeError):
                    pass

                if type(child_1)==node_data and type(child_2)==node_data and child_1 >= child_2:
                    raise ValueError
                parent_node_data.add_children_data(lnode_data, rnode_data)
            parent_node -= 1
