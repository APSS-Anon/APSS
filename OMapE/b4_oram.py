...
from OMapE.b4_tree import OMapTree
...
class OMapE(object):
    ...
    def __getitem__(self, key):
        ...
        nodes_visited, leaf_nodes, access_depth = self.search(h.digest())
    ...
    def build_tree(self, elements=None):
        self.tree = OMapTree.create_tree("1", 2, elements)
    ...
    def retrieve_data(self, depth, node):
        ...
        if node not in current_oram_map:
            ...
            #                              current_oram.position_map, key=current_oram.key, storage_type='ram')
            current_oram_file = self.omap[depth]
    ...
    def search(self, item):
        ...
        while queue:
            ...
            if original_node_data is None:
                ...
                continue
            if current_level != self.tree.depth:

                (lchild, rchild) = original_node_data.get_children()
                # print("Traversal",original_node_data, lchild, rchild,item,original_node_data.left_max_children)
                child = None
                if item <= original_node_data.left_max_children and lchild is not None:
                    next_level_queue.append(lchild)
                elif rchild is not None:
                    next_level_queue.append(rchild)
            elif current_level == self.tree.depth:
                if current_node not in leaf_nodes and item == original_node_data[0]:
                    leaf_nodes.append(original_node_data)
            # if num accesses == total accesses , break loop
            if accesses_made == self.total_accesses:
                queue = []

            if queue == [] and current_level < self.tree.depth:
                accesses_made = 0
                current_level += 1
                ...
                if rest < 0:
                    ...
                    next_level_queue += [2 ** (current_level - 1)] * rest
    ...
    def init_maps(self):
        ...
        for node_id in range(t.root, t.num_nodes + 1):
            ...
            if current_node is None or (type(current_node) == tuple and len(current_node) == 0) \
                    or (type(current_node) == node_data and len(current_node.children) == 0):
                continue
    ...
    def build_oram(self, nodes_map):
        ...
        for (depth, serialized_nodes) in enumerate(nodes_map):
            ...
            self.omap[depth] = PathORAM.setup(file_name, self.block_size, block_count, storage_type='ram')
            for (node_id, blocks_list) in serialized_nodes:
                if node_id not in self.oram_map:
                    self.oram_map[node_id] = []

                for block in blocks_list:
                    self.omap[depth].write_block(block_id, block)
                    # add node to block mapping for client
                    self.oram_map[node_id].append(block_id)
                    block_id = block_id + 1

    #        print("The max size of a block is ",self.max_block_size)

    def apply(self, elements, block_size=128):
...
if __name__ == "__main__":
    ...
    temp_dict = {1: "cat", 2: "dog", 3: "horse", 4: "cow", 5: "pig",
                 6: "chicken", 7: "bird", 8: "hippo", 9: "goldfish"}
    hash_dict = {}
    for element in temp_dict:
        h = SHA256.new()
        h.update(bytes(element))
        hash_dict[h.digest()] = temp_dict[element]
    print(hash_dict)

    omap.apply(temp_dict)
    print(omap[6] == "chicken")
    print(omap[7] == "bird")
    print(omap[3] == "horse")
    print(omap[10] is None)
